...
namespace ctrl_node{
    ...
    // void FSM::mission_triger_callback(const geometry_msgs::PoseStampedConstPtr& msgPtr) {
    //     if(!takeoff_triger_received){
    //         takeoff_triger_received = true;
    //         ROS_INFO("\033[32m[FSM]:Takeoff triger received!\033[32m");
    //     }else{
    ...
    void FSM::init(ros::NodeHandle& nh){
        ...
        att_ang_controller.init(param,nh);

        takeoff_triger_sub_ = nh.subscribe<quadrotor_msgs::TakeoffLand>("takeoff_land", 1, &FSM::takeoff_triger_callback, this, 
                                                                        ros::TransportHints().tcpNoDelay());

        // 
        mission_triger_sub_ = nh.subscribe<geometry_msgs::PoseStamped>("/landing_triger", 1,
        // mission_triger_sub_ = nh.subscribe<geometry_msgs::PoseStamped>("/traj_start_trigger", 1,
                                                                        boost::bind(&Mission_Triger_t::feed, &mission_triger_data, _1),
                                                                        ros::VoidConstPtr(),
                                                                        ros::TransportHints().tcpNoDelay());
        // 
