...
namespace ctrl_node{
    ...
    void FSM::fsm_timer(const ros::TimerEvent& event){
                ...
                if(state_data.current_state.mode == "OFFBOARD"){
                    if(takeoff_triger_received){
                        ...
                        if(state_data.current_state.armed){
                            ...
                            takeoff_triger_received = false; // 已解锁,停止重试
                        ...
                        //     ROS_ERROR("[FSM]:Reject Auto_Takeoff, Odom_Vel=%fm/s, non-static takeoff is not allowed!", odom_data.v.norm());
                            ...
                            if(!toggle_arm_disarm(true)){ //arming rejected
                                // PX4 临时拒绝 ARM 很常见(刚切 OFFBOARD,EKF/offboard 流尚未稳定)→
                                // 保留 takeoff_triger_received,下一周期重试,而不是一次失败就放弃。
                                ROS_ERROR_THROTTLE(1.0, "[FSM]:takeoff arming rejected by PX4, retrying...");
                            }else{
                                current_state = TAKEOFF;
                                att_ang_controller.resetThrustMapping();
                                set_start_pose_for_takeoff(odom_data);
                                takeoff_triger_received = false; // 成功起飞,清除标志
        ...
        case 2:           
            debug_msg = att_ang_controller.calculateControl(des, mission_triger_data, odom_data, imu_data, u, now_time);
            publish_attitude_ctrl(u, now_time);
            break;
            
        case 3:
            // if(current_state == MANUAL){
            //     debug_msg = att_ang_controller.calculateControl(des, mission_triger_data, odom_data, imu_data, u);
            // }else{
                debug_msg = att_ang_controller.calculateControlCMD(des, mission_triger_data, odom_data, imu_data, u, now_time);
            // }
        ...
        // takeoff_triger_received 不再每周期无条件清零;改为仅在起飞成功/已解锁时清除(见 MANUAL 分支),
        // 以支持 PX4 临时拒绝 ARM 后自动重试。
    }
    ...
    void FSM::publish_attitude_ctrl(const Controller::Controller_Output_t &u, const ros::Time &stamp){
        ...
        msg.body_rate.z = u.bodyrates.z(); // yaw_rate 前馈
        msg.thrust = u.thrust;
    ...
    bool FSM::toggle_arm_disarm(bool arm){
        ...
        {
            // result 为 PX4 返回的 MAV_RESULT: 0=ACCEPTED, 1=TEMPORARILY_REJECTED(可重试),
            // 2=DENIED(预飞检查未通过,如 EKF/GPS/传感器校准), 3=NOT_SUPPORTED, 4=IN_PROGRESS
            ROS_ERROR("[%s] rejected by PX4! (mavros success=%d, MAV_RESULT=%d)",
                      arm ? "ARM" : "DISARM", arm_cmd.response.success, arm_cmd.response.result);
            return false;
        }
    ...
    void FSM::set_hov_with_odom(){
        ...
        // hover_pose(3) = Controller::q2yaw(odom.q); // get yaw
    ...
    void FSM::set_start_pose_for_takeoff(const Odom_Data_t &odom){
        ...
        // takeoff_state.start_pose(2) = 0.0;
