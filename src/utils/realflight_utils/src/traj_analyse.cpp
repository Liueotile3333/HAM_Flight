...
namespace trajAnalyse
{
    ...
    private:
        ...
        void posedifferPublish()
        {
            ...
            posdiffer.data = sqrt((gtruth.pose.position.x - des.pos.position.x) * (gtruth.pose.position.x - des.pos.position.x) + (gtruth.pose.position.y - des.pos.position.y) * (gtruth.pose.position.y - des.pos.position.y) + (gtruth.pose.position.z - des.pos.position.z) * (gtruth.pose.position.z - des.pos.position.z));
        ...
        void analyse(const ros::TimerEvent &)
        {
    ...
    public:
        void onInit(void)
        {
            // init() 只注册 ROS 接口且不会阻塞；同步初始化避免 nodelet 卸载时
            // joinable std::thread 析构触发 std::terminate 杀死共享 manager。
