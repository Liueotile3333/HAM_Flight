...
class OdometryConverter:
    ...
    def __init__(self, frame_id_in, frame_id_out, broadcast_tf,
    ...
    def in_odom_callback(self, in_odom_msg):
        ...
        if in_odom_msg.header.frame_id != self.frame_id_in:
            rospy.logwarn_throttle(
                1.0,
                'Unexpected frame_id [%s], expected [%s]; message dropped.',
                in_odom_msg.header.frame_id,
                self.frame_id_in,
            )
            return
    ...
    def path_pub_callback(self, _event):
        if self.path:
            path = Path()
    ...
    def tf_pub_callback(self, _event):
        self.tf_pub_flag = True
