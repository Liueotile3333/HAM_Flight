#ifndef CHCNAV_HC_MSG_PARSER_NODE_HPP_
#define CHCNAV_HC_MSG_PARSER_NODE_HPP_
...
public:
    ...
    hc__device_connector *device_connector = nullptr; // 设备连接器
                        ...
                        const std::string &node_name)
    {
        if (NULL != nh && NULL != private_nh && NULL != device_connector)
        {
            ...
            // 初始化解析器
            if (hc__init_msg_parser(&this->parser, HC_PROTOCOL_MAX_LEN,
                                    parser_read_handler, this) != 0)
            {
                ROS_ERROR("hc message parser initialization failed");
                this->state = -1;
                return;
            }
    ...
    void node_start(void)
    {
        ...
        while (ros::ok()) // 此处循环表示直到成功取到一次数据（指通过校验，hc cgi 自定义协议或者 nmea 协议）后才 break 该层循环进入 sleep
        {
            while (1)
            {
                const int scan_result = hc__msg_parser_scan(&this->parser, &this->token);
                if (scan_result == HC__SCAN_NO_DATA)
                {
                    // 退出内部取包循环，由外层 ros::Rate 休眠并检查 ros::ok()。
                    break;
                }
                if (scan_result == HC__SCAN_ERROR)
                {
                    ROS_ERROR_THROTTLE(1.0, "[parser] Device read/parser error.");
                ...
                switch (token.type)
                {
                    case HC__TOKERN_HC_SHORT_MSG:
                        ...
                        single_msg.type_value.cgi_header.msg_id =
                            (static_cast<unsigned int>(static_cast<unsigned char>(this->token.token_value.value[7])) << 8U) |
                            static_cast<unsigned int>(static_cast<unsigned char>(this->token.token_value.value[6]));
     ...
     * @brief 获取私有对象节点名称
     * */
    std::string get_node_name(void) { return this->node_name; }
    ...
    void node_write(const chcnav::int8_array::ConstPtr &msg)
    {
        if (msg->data.empty())
        {
            ROS_WARN_THROTTLE(1.0, "empty device write request ignored");
            return;
        }
        this->device_connector->write(
    ...
    ~hc__msg_parser_node()
    {
        hc__deinit_msg_parser(&this->parser);
        if (this->device_connector != nullptr)
        {
...
int parser_read_handler(void *buffer, size_t size, size_t *size_read, void *parser_id)
{
    ...
    const int read_size = parser_node->device_connector->read(
    ...
    *size_read = static_cast<size_t>(read_size);
    return 0;
};
...
#endif  // CHCNAV_HC_MSG_PARSER_NODE_HPP_
