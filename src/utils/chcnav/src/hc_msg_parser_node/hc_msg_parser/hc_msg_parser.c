...
{
    if (parser == NULL || buffer_size == 0U || read_handler == NULL)
        return -1;
    ...
    if (parser->buffer.value == NULL)
    {
        parser->buffer.end = 0U;
        snprintf(parser->error.description,
                 sizeof(parser->error.description),
                 "parser buffer allocation failed");
        return -1;
    }
...
void hc__deinit_msg_parser(hc__msg_parser_t *parser)
{
    if (parser == NULL)
        return;

    free(parser->buffer.value);
    parser->buffer.value = NULL;
    parser->buffer.end = 0U;
    parser->buffer.pointer = 0U;
    parser->buffer.last = 0U;
}
    ...
    while (1)
    {
        if (parser->buffer.pointer == parser->buffer.last)
        {
            ...
            const int read_result =
                parser->read_handler((void *)parser->buffer.value,
                                     parser->buffer.end,
                                     &parser->buffer.last,
                                     parser->parser_id);
            parser->buffer.pointer = 0;
            ...
            // EOF/暂时无数据均交还上层，由 ros::Rate 休眠并重新检查 ros::ok()。
            if (parser->buffer.last == 0U)
            {
                return HC__SCAN_NO_DATA;
            }
        }
...
static int hc__msg_deal_S19(hc__msg_parser_t *parser, hc__msg_token_t *token, unsigned char c)
{
    const unsigned char head_0 = token->token_value.value[1];
    const unsigned char head_1 = token->token_value.value[2];
    const int supported_head =
        (head_0 == 'G' && head_1 == 'P') ||
        (head_0 == 'G' && head_1 == 'N') ||
        (head_0 == 'B' && head_1 == 'D') ||
        (head_0 == 'G' && head_1 == 'T');
    ...
    else
    {
        ...
        snprintf(parser->error.description, sizeof(parser->error.description),
                 "unsupported nmea head [$%c%c]", head_0, head_1);
...
{
    (void)c;
    if (token->token_value.value_len >= 2 &&
        token->token_value.value[token->token_value.value_len - 2] == '\r' &&
        token->token_value.value[token->token_value.value_len - 1] == '\n')
    {
    ...
    else
    {
        ...
        const int end_0 = token->token_value.value_len >= 2
                              ? token->token_value.value[token->token_value.value_len - 2]
                              : -1;
        const int end_1 = token->token_value.value_len >= 1
                              ? token->token_value.value[token->token_value.value_len - 1]
                              : -1;
        snprintf(parser->error.description, sizeof(parser->error.description),
                 "error nmea end [0x%x 0x%x]", end_0, end_1);
