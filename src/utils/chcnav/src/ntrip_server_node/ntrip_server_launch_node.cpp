...
int main(int argc, char *argv[])
{
    ...
    if (login_type.compare("user_password") == 0)
    {
        ...
        ROS_INFO("NTRIP user/password login configured (credentials redacted).");
    ...
    else if (login_type.compare("sn_device") == 0)
    {
        ...
        ROS_INFO("NTRIP SN-device login configured (credentials redacted, coordinate=%s).",
    ...
    else if (login_type.compare("third_cors") == 0)
    {
        ...
        ROS_INFO("Third-party CORS configured: host=%s port=%s mountpoint=%s (credentials redacted).",
                 host.c_str(), port.c_str(), mountpoint.c_str());
    }
    else
    {
