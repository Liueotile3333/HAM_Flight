...
protected:
    ...
    std::atomic<swas_char_t> swas_status{SWAS_FALSE};
    std::atomic<swas_char_t> auth_success_flag{SWAS_FALSE};
    std::atomic<swas_char_t> calib_success_flag{SWAS_FALSE};
    std::atomic<swas_char_t> socket_connect_flag{SWAS_FALSE};

    template <std::size_t N>
    static void copy_fixed(swas_uint8_t (&destination)[N],
                           const std::string &source)
    {
        static_assert(N > 0U, "SDK text destination must not be empty");
        std::memset(destination, 0, N);
        const std::size_t copy_length =
            std::min(source.size(), N - 1U);
...
public:
     ...
     * @param coordinate
     * */
    void set_sn_device(const std::string &sn, const std::string &snkey,
                       const std::string &app_id,
                       swas_sdk_coordinate_value_e coordinate)
    {
        this->sdk_config.login_type = SWAS_SDK_LOGIN_TYPE_SN_DEVICE_ID;
        copy_fixed(this->sdk_config.sn_land.sn, sn);
        copy_fixed(this->sdk_config.key, snkey);
        copy_fixed(this->sdk_config.appID, app_id);
    ...
    void set_user_password(const std::string &username,
                           ...
                           const std::string &snkey,
                           const std::string &app_id,
                           swas_sdk_coordinate_value_e coordinate)
    {
        this->sdk_config.login_type = SWAS_SDK_LOGIN_TYPE_USER_PASSWORD;
        copy_fixed(this->sdk_config.username_password.username, username);
        copy_fixed(this->sdk_config.username_password.password, password);
        copy_fixed(this->sdk_config.key, snkey);
        copy_fixed(this->sdk_config.appID, app_id);
        this->sdk_config.coordinate_type = coordinate;
    }
     ...
     * @param password
     * */
    void set_third_cors(const std::string &host, swas_uint16_t port,
                        const std::string &mountpoint,
                        const std::string &username,
                        const std::string &password)
    {
        this->sdk_config.login_type = SWAS_SDK_LOGIN_TYPE_THIRD_CORS;
        copy_fixed(this->sdk_config.third_cors.third_addr.sin_addr, host);
        this->sdk_config.third_cors.third_addr.sin_port = port;
        copy_fixed(this->sdk_config.third_cors.mountpoint, mountpoint);
        copy_fixed(this->sdk_config.third_cors.username, username);
        copy_fixed(this->sdk_config.third_cors.password, password);
    }
    ...
    swas_int32_t disconnect(swas_void_t)
    {
        ...
        this->swas_status.store(SWAS_FALSE);
        this->auth_success_flag.store(SWAS_FALSE);
        this->calib_success_flag.store(SWAS_FALSE);
        this->socket_connect_flag.store(SWAS_FALSE);
    ...
    {
        this->socket_connect_flag.store(status);
    }
    void set_calib_success_flag(swas_char_t status)
    {
        this->calib_success_flag.store(status);
    }
    void set_auth_success_flag(swas_char_t status)
    {
        this->auth_success_flag.store(status);
    }
    void set_swas_status(swas_char_t status)
    {
        this->swas_status.store(status);
    }
    ...
    {
        return this->socket_connect_flag.load();
    }
    swas_char_t get_calib_success_flag(void)
    {
        return this->calib_success_flag.load();
    }
    swas_char_t get_auth_success_flag(void)
    {
        return this->auth_success_flag.load();
    }
    swas_char_t get_swas_status(void)
    {
        return this->swas_status.load();
    }
